...
namespace OnlineOutPatientManagementSystem.UI.Prescription
{
	public partial class frmTest : System.Web.UI.Page
	{
		...
		private void InitializeDataSource()
		{
			...
			dtPerson.Columns.Add("ChiefComplain");
			dtPerson.Columns.Add("DurationYY");
			dtPerson.Columns.Add("DurationMM");
			dtPerson.Columns.Add("DurationDD");


			// Specify PersonID column as an auto increment column
			// and set the starting value and increment.
			dtPerson.Columns["ID"].AutoIncrement = true;
			dtPerson.Columns["ID"].AutoIncrementSeed = 1;
			dtPerson.Columns["ID"].AutoIncrementStep = 1;

			// Set PersonID column as the primary key.
			DataColumn[] dcKeys = new DataColumn[1];
			dcKeys[0] = dtPerson.Columns["ID"];
			dtPerson.PrimaryKey = dcKeys;

			// Add new rows into the DataTable.
			dtPerson.Rows.Add(null, "AAA", "AAA", "AAA", "AAA");
		...
		protected void gvPerson_RowUpdating(object sender, GridViewUpdateEventArgs e)
		{
			if (ViewState["dtPerson"] != null)
			{
				...
				//TextBox ChiefComplain = gvPerson.Rows[e.RowIndex].FindControl("ChiefComplain") as TextBox;
				//TextBox DurationYY = gvPerson.Rows[e.RowIndex].FindControl("DurationYY") as TextBox;
				//TextBox DurationMM = gvPerson.Rows[e.RowIndex].FindControl("DurationMM") as TextBox;
				//TextBox DurationDD = gvPerson.Rows[e.RowIndex].FindControl("DurationDD") as TextBox;


				//// Retrieve edited values and updating respective items.
				//drPerson["ChiefComplain"] = gvPerson.Rows[e.RowIndex].FindControl("TextBox1.Text") as TextBox;
				//drPerson["DurationYY"] = gvPerson.Rows[e.RowIndex].FindControl("TextBox2.Text") as TextBox;
				//drPerson["DurationMM"] = gvPerson.Rows[e.RowIndex].FindControl("TextBox3.Text") as TextBox;
				//drPerson["DurationDD"] = gvPerson.Rows[e.RowIndex].FindControl("TextBox4.Text") as TextBox;

				// Retrieve edited values and updating respective items.
				drPerson["ChiefComplain"] = ((TextBox)gvPerson.Rows[e.RowIndex].FindControl("TextBox1")).Text;
				drPerson["DurationYY"] = ((TextBox)gvPerson.Rows[e.RowIndex].FindControl("TextBox2")).Text;
				drPerson["DurationMM"] = ((TextBox)gvPerson.Rows[e.RowIndex].FindControl("TextBox3")).Text;
				drPerson["DurationDD"] = ((TextBox)gvPerson.Rows[e.RowIndex].FindControl("TextBox4")).Text;
		...
		protected void lbtnAdd_Click(object sender, EventArgs e)
		{
			if (IsPostBack)
			{
				...
				DropDownList1.DataSource = BL.Classes.LookUp.GetChiefComplainces();
				DropDownList1.DataTextField = "ChiefComplaince";
				DropDownList1.DataValueField = "CCCode";
		...
		protected void lbtnSubmit_Click(object sender, EventArgs e)
		{
			if (ViewState["dtPerson"] != null)
			{
				...
				// Add the new row.
				dtPerson.Rows.Add(null, DropDownList1.SelectedItem, txtDurationYY.Text, txtDurationMM.Text,txtDurationDD.Text);
